...
char HumanPlayerStrategy::choice(const std::string& type){
    char choice = 'a';
...
void HumanPlayerStrategy::reinforcePhase() {
    ...
    for (Territory* t : defendList) {
        ...
        // Build the prompt as a std::string
        std::string prompt = "Do you want to deploy to " + t->getName() + "? (y/n): ";
        char choice = this->choice(prompt);

        if (choice == 'n') {
            // Ask if they want to keep deploying to other territories
            choice = this->choice("Do you want to continue deploying? (y/n): ");
            if (choice == 'n') {
                break;
            }
            continue;
        ...
        // If they answered 'y', ask how many armies to deploy
        while (true) {
            std::cout << "You have " << player_->reinforcementPool()
                      << " reinforcements left.\n";

            int num = 0;
            std::cout << "Enter number of reinforcements to deploy to "
                      << t->getName() << ": ";
            std::cin >> num;

            if (num > player_->reinforcementPool() || num <= 0) {
                std::cout << "Invalid number of reinforcements. Try again.\n";
                continue;
            }

            // Add deploy order and update pool
...
void HumanPlayerStrategy::attackPhase() {
    ...
    for (Territory* target : attackList) {
        ...
        std::string prompt = "Do you want to attack " + target->getName() + "? (y/n): ";
        char choice = this->choice(prompt);
        ...
        // Gather possible territories to attack from
        std::cout << "Territories to attack from:\n";
        std::vector<Territory*> attackFrom;
        int idx = 0;

        for (Territory* adj : target->getAdjacentTerritories()) {
            if (adj->getPlayer() == player_) {
                attackFrom.push_back(adj);
                std::cout << "[" << idx++ << "] " << *adj << std::endl;
            }
        }

        if (attackFrom.empty()) {
            std::cout << "No valid territories to attack from.\n";
            continue;
        }

        // Let the user choose which territory to attack from
        int selectedIndex = -1;
        while (true) {
            std::cout << "Select territory to attack from (index): ";
            std::cin >> selectedIndex;

            if (selectedIndex < 0 || selectedIndex >= (int)attackFrom.size()) {
        ...
        Territory* fromTerritory = attackFrom[selectedIndex];

        // For now, move all armies from the chosen territory
        int numArmies = fromTerritory->getArmy();
        if (numArmies <= 0) {
            std::cout << "No armies available to attack from "
                      << fromTerritory->getName() << ".\n";
            continue;
        }

        player_->getOrders()->addOrder(
            new AdvanceOrder(player_, fromTerritory, target, numArmies)
        );
    }
}
