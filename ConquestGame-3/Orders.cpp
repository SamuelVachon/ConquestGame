...
void AdvanceOrder::execute() {
    ...
    // Change the player type from neutral to aggressive if they were attacked
    Player* defendingPlayer = defender; // the old owner
    if (defendingPlayer) {
        PlayerStrategy* strat = defendingPlayer->getStrategy();
        if (dynamic_cast<NeutralPlayerStrategy*>(strat) != nullptr) {
            defendingPlayer->setStrategy(new AggressivePlayerStrategy(defendingPlayer));
        }
    }
